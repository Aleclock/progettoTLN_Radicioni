# **FRAME: Container_focused_placing**

> An Agent or Cause affects a Goal location by moving a Theme, but without necessarily filling or covering it.  This frame is for Inheritance purposes.

<br/>

# **Frame name: Container** <br/><br/>

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('container.n.01')```| - | X |any object that can be used to hold things (especially a large metal boxlike object of standardized dimensions that can be loaded from one form of transport to another)

<br/><br/>

# **Frame elements** 

<br/><br/>

* ## Agent

> The Agent is the actor who instigates the filling.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('agent.n.01')``` |0.188 | X | an active and efficient cause; capable of producing a certain effect
```Synset('agent.n.02')``` |0.188 | | a representative who acts on behalf of other persons or organizations
```Synset('agent.n.03')``` |0.188 | | a substance that exerts some force or effect
```Synset('agent.n.04')``` |0.125 | | a businessman who buys or sells for another in exchange for a commission
```Synset('agent.n.05')``` |0.125 | | any agent or representative of a federal agency or bureau
```Synset('agentive_role.n.01')``` |0.125 | | the semantic role of the animate entity that instigates or causes the happening denoted by the verb in the clause

<br/><br/>

* ## Theme

> The Theme is the physical object or substance which changes location. 'Rev. Lovejoy carefully brushed the rolls with butter. '

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('subject.n.01')``` |0.125 | X | the subject matter of a conversation or discussion
```Synset('theme.n.02')``` |0.125 | | a unifying idea that is a recurrent element in literary or artistic work
```Synset('theme.n.03')``` |0.125 | | (music) melodic subject of a musical composition
```Synset('composition.n.08')``` |0.062 | | an essay (especially one written as an assignment)
```Synset('root.n.03')``` |0.062 | | (linguistics) the form of a word after all affixes are removed
```Synset('theme.v.01')``` |0.125 | | provide with a particular theme or motive

<br/><br/>

* ## Source

> The Source is the location where the Theme begins its motion. 'Josef filled his flask from the whiskey bottle.'

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('beginning.n.04')``` |0.125 | X | the place where something begins, where it springs into being
```Synset('source.n.02')``` |0.062 | | a document (or organization) from which information is obtained
```Synset('source.n.03')``` |0.062 | | anything that provides inspiration for later work
```Synset('source.n.04')``` |0.062 | | a facility where something is available
```Synset('informant.n.01')``` |0.062 | | a person who supplies information
```Synset('generator.n.03')``` |0.125 | | someone who originates or causes or initiates something
```Synset('source.n.07')``` |0.062 | | (technology) a process by which energy or a substance enters a system
```Synset('reservoir.n.04')``` |0.125 | | anything (a person or animal or plant or substance) in which an infectious agent normally lives and multiplies
```Synset('reference.n.08')``` |0.062 | | a publication (or a passage from a publication) that is referred to
```Synset('source.v.01')``` |0.062 | | get (a product) from another country or business
```Synset('source.v.02')``` |0.062 | | specify the origin of

<br/><br/>

* ## Path

> The Path is the trajectory of the motion.  'Jess filled the container through a small hole in the lid.'

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('way.n.05')``` |0.062 | | a course of conduct
```Synset('path.n.02')``` |0.062 | | a way especially designed for a particular use
```Synset('path.n.03')``` |0.188 | | an established line of travel or access
```Synset('path.n.04')``` |0.188 | X | a line or route along which something travels or moves

<br/><br/>

* ## Goal

> The Goal is the area or container being filled. Goal is generally the NP Object in this frame.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('goal.n.01')``` |0.188 | X | the state of affairs that a plan is intended to achieve and that (when achieved) terminates behavior intended to achieve it
```Synset('finish.n.04')``` |0.125 | | the place designated as the end (as of a race or journey)
```Synset('goal.n.03')``` |0.125 | | game equipment consisting of the place toward which players of a game try to advance a ball or puck in order to score points
```Synset('goal.n.04')``` |0.125 | | a successful attempt at scoring

<br/><br/>

* ## Degree

> Degree to which the filling occurs.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('degree.n.01')``` |0.062 | X | a position on a scale of intensity or amount or quality
```Synset('degree.n.02')``` |0.062 | | a specific identifiable position in a continuum or series or especially in a process
```Synset('academic_degree.n.01')``` |0.062 | | an award conferred by a college or university signifying that the recipient has satisfactorily completed a course of study
```Synset('degree.n.04')``` |0.062 | | a measure for arcs and angles
```Synset('degree.n.05')``` |0.062 | | the highest power of a term or variable
```Synset('degree.n.06')``` |0.062 | | a unit of temperature on a specified scale
```Synset('degree.n.07')``` |0.062 | | the seriousness of something (e.g., a burn or crime)

<br/><br/>

* ## Depictive

> Depictive phrase describing the Agent of the filling action. 'Naked, Margeret filled the tub.'

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('delineative.s.01')``` |0.062 | X | depicted in a recognizable manner

<br/><br/>

* ## Manner

> The Manner in which the filling occurs.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('manner.n.01')``` |0.062 | X | how something is done or how it happens
```Synset('manner.n.02')``` |0.062 | | a way of acting or behaving
```Synset('manner.n.03')``` |0.062 | | a kind

<br/><br/>

* ## Means

> The Means by which the filling occurs.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('means.n.01')``` |0.188 | | how a result is obtained or an end is achieved
```Synset('means.n.02')``` |0.062 | X | an instrumentality for accomplishing some end
```Synset('means.n.03')``` |0.062 | | considerable capital (wealth or income)
```Synset('mean.n.01')``` |0.125 | | an average of n numbers computed by adding some function of the numbers and dividing by some function of n
```Synset('mean.v.01')``` |0.188 | | mean or intend to express or convey
```Synset('entail.v.01')``` |0.125 | | have as a logical consequence
```Synset('mean.v.03')``` |0.125 | | denote or connote
```Synset('intend.v.01')``` |0.125 | | have in mind as a purpose
```Synset('mean.v.05')``` |0.062 | | have a specified degree of importance
```Synset('think_of.v.04')``` |0.062 | | intend to refer to
```Synset('mean.v.07')``` |0.125 | | destine or designate for a certain purpose

<br/><br/>

* ## Result

> The Result is the resultant state after the filling has occurred.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('consequence.n.01')``` |0.312 | X | a phenomenon that follows and is caused by some previous phenomenon
```Synset('solution.n.02')``` |0.062 | | a statement that solves a problem or explains how to solve the problem
```Synset('result.n.03')``` |0.062 | | something that results
```Synset('resultant_role.n.01')``` |0.062 | | the semantic role of the noun phrase whose referent exists only by virtue of the activity denoted by the verb in the clause
```Synset('result.v.01')``` |0.062 | | issue or terminate (in a specified way, state, etc.); end
```Synset('leave.v.07')``` |0.188 | | have as a result or residue
```Synset('result.v.03')``` |0.125 | | come about or follow as a consequence

<br/><br/>

* ## Explanation

> The Explanation for which the Agent fills or covers the Goal. 'Samantha covered the table with flowers because she was bored.'

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('explanation.n.01')``` |0.125 | X | a statement that makes something comprehensible by describing the relevant structure or operation or circumstances etc.
```Synset('explanation.n.02')``` |0.062 | | thought that makes something comprehensible
```Synset('explanation.n.03')``` |0.062 | | the act of explaining; making something plain or intelligible

<br/><br/>

* ## Time

> When the filling occurs.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('time.n.01')``` |0.062 | | an instance or single occasion for some event
```Synset('time.n.02')``` |0.062 | X | a period of time considered as a resource under your control and sufficient to accomplish something
```Synset('time.n.03')``` |0.062 | | an indefinite period (usually marked by specific attributes or activities)
```Synset('time.n.04')``` |0.062 | | a suitable moment
```Synset('time.n.05')``` |0.25 | | the continuum of experience in which events pass from the future through the present to the past
```Synset('time.n.06')``` |0.062 | | a person's experience on a particular occasion
```Synset('clock_time.n.01')``` |0.062 | | a reading of a point in time as given by a clock
```Synset('fourth_dimension.n.01')``` |0.062 | | the fourth coordinate that is required (along with three spatial dimensions) to specify a physical event
```Synset('meter.n.04')``` |0.062 | | rhythm as given by division into parts of equal duration
```Synset('prison_term.n.01')``` |0.062 | | the period of time a prisoner is imprisoned
```Synset('clock.v.01')``` |0.062 | | measure the time or duration of an event or action or the person who performs an action in a certain period of time
```Synset('time.v.02')``` |0.062 | | assign a time for an activity or event
```Synset('time.v.03')``` |0.062 | | set the speed, duration, or execution of
```Synset('time.v.04')``` |0.062 | | regulate or set the time of
```Synset('time.v.05')``` |0.062 | | adjust so that a force is applied and an action occurs at the desired time

<br/><br/>

* ## Place

> Where the filling takes place.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('topographic_point.n.01')``` |0.125 | | a point located with respect to surface features of some region
```Synset('place.n.02')``` |0.125 | | any area set aside for a particular purpose
```Synset('place.n.03')``` |0.125 | | an abstract mental location
```Synset('place.n.04')``` |0.062 | | a general vicinity
```Synset('stead.n.01')``` |0.125 | | the post or function properly or customarily occupied or served by another
```Synset('place.n.06')``` |0.062 | | a particular situation
```Synset('home.n.01')``` |0.062 | | where you live at a particular time
```Synset('position.n.06')``` |0.062 | | a job in an organization
```Synset('position.n.01')``` |0.125 | | the particular portion of space occupied by something
```Synset('place.n.10')``` |0.062 | | proper or designated social situation
```Synset('seat.n.01')``` |0.125 | | a space reserved for sitting (as in a theater or on a train or airplane)
```Synset('place.n.12')``` |0.062 | | the passage that is being read
```Synset('place.n.13')``` |0.125 | | proper or appropriate position or location
```Synset('plaza.n.01')``` |0.062 | | a public square with room for pedestrians
```Synset('place.n.15')``` |0.125 | | an item on a list or in a sequence
```Synset('space.n.07')``` |0.062 | | a blank area
```Synset('put.v.01')``` |0.5 | | put into a certain place or abstract location
```Synset('place.v.02')``` |0.125 | | place somebody in a particular situation or location
```Synset('rate.v.01')``` |0.062 | | assign a rank or rating to
```Synset('locate.v.03')``` |0.188 | | assign a location to
```Synset('place.v.05')``` |0.062 | | to arrange for
```Synset('place.v.06')``` |0.125 | | take a place in a competition; often followed by an ordinal
```Synset('target.v.01')``` |0.25 | | intend (something) to move towards a certain goal
```Synset('identify.v.01')``` |0.062 | | recognize as being; establish the identity of someone or something
```Synset('place.v.09')``` |0.062 | | assign to (a job or a home)
```Synset('set.v.09')``` |0.125 | | locate
```Synset('place.v.11')``` |0.062 | | estimate
```Synset('place.v.12')``` |0.125 | | identify the location or place of
```Synset('invest.v.01')``` |0.125 | | make an investment
```Synset('station.v.01')``` |0.312 | X | assign to a station
```Synset('place.v.15')``` |0.062 | | finish second or better in a horse or dog race
```Synset('place.v.16')``` |0.062 | | sing a note with the correct pitch

<br/><br/>

* ## Purpose

> The purpose for which the Agent fills the Goal. 'Samantha covered the table with flowers to please her mother.' 

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('purpose.n.01')``` |0.125 | | an anticipated outcome that is intended or that guides your planned actions
```Synset('function.n.02')``` |0.125 | X | what something is used for
```Synset('determination.n.02')``` |0.125 | | the quality of being determined to do or achieve something; firmness of purpose
```Synset('aim.v.02')``` |0.125 | | propose or intend
```Synset('purpose.v.02')``` |0.062 | | reach a decision

<br/><br/>

* ## Instrument

> The Instrument with which the filling occurs.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('instrument.n.01')``` |0.125 | X | a device that requires skill for proper use
```Synset('instrument.n.02')``` |0.062 | | the means whereby some act is accomplished
```Synset('instrument.n.03')``` |0.125 | | a person used by another to gain an end
```Synset('legal_document.n.01')``` |0.125 | | (law) a document that states some contractual relationship or grants some right
```Synset('instrumental_role.n.01')``` |0.125 | | the semantic role of the entity (usually inanimate) that the agent uses to perform an action or start a process
```Synset('musical_instrument.n.01')``` |0.188 | | any of various devices or contrivances that can be used to produce musical tones or sounds
```Synset('instrument.v.01')``` |0.125 | | equip with instruments for measuring, recording, or controlling
```Synset('instrument.v.02')``` |0.062 | | write an instrumental score for
```Synset('instrument.v.03')``` |0.062 | | address a legal document to

<br/><br/>

* ## Subregion

> The Subregion is the smaller area of the Goal where the filling or covering occurs. 'Mrs. Bouvier anointed the children with oil on the forehead.'

<br/><br/>

* ## Cause

> An event which brings about the filling of the Goal.

Synset | Score | Best synset | Definition
------------ | :------------: | :-------------: | -------------
```Synset('cause.n.01')``` |0.125 | | events that provide the generative force that is the origin of something
```Synset('cause.n.02')``` |0.125 | | a justification for something existing or happening
```Synset('campaign.n.02')``` |0.125 | | a series of actions advancing a principle or tending toward a particular end
```Synset('causal_agent.n.01')``` |0.25 | X | any entity that produces an effect or is responsible for events or results
```Synset('lawsuit.n.01')``` |0.062 | | a comprehensive term for any proceeding in a court of law whereby an individual seeks a legal remedy
```Synset('cause.v.01')``` |0.25 | | give rise to; cause to happen or occur, not always intentionally
```Synset('induce.v.02')``` |0.312 | | cause to do; cause to act in a specified manner

<br/><br/>

# **Lexical units**

<br/><br/>

* ## No lexical units